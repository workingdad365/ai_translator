...
- **OpenRouter 실행 제공자 지정**: OpenRouter 선택 시 모델을 실제로 호스팅하는 실행 제공자의 slug(예: `groq`, `deepinfra/turbo`)를 선택적으로 저장함. 지정하면 요청의 `provider.only`로 전달하여 해당 실행 제공자로 제한하고, 비워 두면 OpenRouter 자동 라우팅을 사용함. LiteLLM은 실행 제공자 지정을 사용하지 않고 프록시의 라우팅을 그대로 씀.
...
LiteLLM 프록시(예: `https://litellm.example.com`)를 OpenAI 호환 엔드포인트로 사용함. 확장 프로그램은 프록시의 `/v1/chat/completions` 경로를 호출하며, API 키에는 LiteLLM 가상 키(`sk-...`)를 입력함. 모델명에는 LiteLLM에 등록한 Public Model Name을 쓰며, `/v1/models` 경로를 조회하므로 팝업의 **모델 가져오기** 버튼으로 해당 키에 허용된 모델 목록을 받을 수 있음.
